...
class GroupedTimeEncoding(nn.Module):
    def __init__(self, total_channels=552, num_phases=23, embed_dim=64):
        super().__init__()
        self.num_phases = num_phases
        self.embed_dim = embed_dim
        self.time_embed = nn.Parameter(torch.randn(1, num_phases, embed_dim))
        # 可选的投影层（如果需要降维）
        self.proj = nn.Linear(embed_dim, 1) if embed_dim > 1 else nn.Identity()

    def forward(self, x):
        B, C, H, W = x.shape  # [B, 552, H, W]

        # 方案1：通道维度相加（推荐）
        time_weights = self.time_embed.repeat_interleave(24, dim=1)  # [1, 552, embed_dim]
        time_weights = self.proj(time_weights).unsqueeze(-1)  # [1, 552, 1, 1]
...
class ChangeDetectionMamba(nn.Module):
    def __init__(self, config):
        ...
        self.time_encoder = GroupedTimeEncoding(
            total_channels=552,
            num_phases=23,
            embed_dim=64,
        )
    ...
    def forward(self, x_2010, x_2015):
        ...
        feats_2010 = self.time_encoder(feats_2010)
        feats_2015 = self.time_encoder(feats_2015)

        feats_2010 = feats_2010.reshape(B, C, H*W)
        feats_2015 = feats_2015.reshape(B, C, H*W)

        feats_2010 = self.channel_mamba_blocks(feats_2010)
        feats_2015 = self.channel_mamba_blocks(feats_2015)
