...
class ChangeDetectionMamba(nn.Module):
    ...
    def forward(self, x_2010, x_2015):
        ...
        feats_2010 = feats_2010.reshape(B, C, H*W).reshape(B, 23, C//23, H*W).reshape(B, 23, C//23*H*W)
        feats_2015 = feats_2015.reshape(B, C, H*W).reshape(B, 23, C//23, H*W).reshape(B, 23, C//23*H*W)

        feats_2010 = self.temporal_mamba(feats_2010)
        feats_2015 = self.temporal_mamba(feats_2015)

        feats_2010 = feats_2010.reshape(B, 23, C//23, H * W).permute(0, 2, 1, 3).reshape(B, C//23, 23*H*W)
        feats_2015 = feats_2015.reshape(B, 23, C//23, H * W).permute(0, 2, 1, 3).reshape(B, C//23, 23*H*W)

        feats_2010 = self.spectral_mamba(feats_2010)
        feats_2015 = self.spectral_mamba(feats_2015)

        feats_2010 = feats_2010.reshape(B, C//23, 23, H*W).reshape(B, C, H*W)
        feats_2015 = feats_2015.reshape(B, C//23, 23, H*W).reshape(B, C, H*W)

        feats_2010 = self.input_proj(feats_2010.permute(0, 2, 1))+feats_2010.permute(0, 2, 1)
        feats_2015 = self.input_proj(feats_2015.permute(0, 2, 1))+feats_2015.permute(0, 2, 1)

        feats_2010 = self.channel_mamba(feats_2010.permute(0, 2, 1))
        feats_2015 = self.channel_mamba(feats_2015.permute(0, 2, 1))
