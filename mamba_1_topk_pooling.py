...
class SparseDeformableMambaBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        topk_values, topk_idx = torch.topk(sim, k=k, dim=-1)
        ...
        # 返回topk_indices以便后续pooling使用
        self.topk_indices = topk_idx
...
class ChangeDetectionMamba(nn.Module):
    ...
    def forward(self, x_2010, x_2015):
        ...
        # 前向传播并获取topk_indices
        feats_2010 = self.mamba_blocks(feats_2010)
        topk_indices_2010 = self.mamba_blocks[-1].topk_indices

        feats_2015 = self.mamba_blocks(feats_2015)
        topk_indices_2015 = self.mamba_blocks[-1].topk_indices

        # 只对选中的topk tokens进行平均池化
        pooled_2010 = torch.stack([
            feats_2010[i, topk_indices_2010[i]].mean(dim=0)
            for i in range(B)
        ])

        pooled_2015 = torch.stack([
            feats_2015[i, topk_indices_2015[i]].mean(dim=0)
            for i in range(B)
        ])
...
if __name__ == "__main__":
    ...
    dummy_input_2010 = torch.randn(512, 138, 3, 3).cuda()
    dummy_input_2015 = torch.randn(512, 138, 3, 3).cuda()
