...
from mamba_spatial_channel_parallel_spectral_2_bidirectional_transform import ChangeDetectionMamba
...
config = {
    ...
    'training': {
        ...
        'loss_weights': {
            ...
            'landcover': 1.0,  # LC loss multiplier
...
def adversarial_loss(model, x, targets, epsilon=0.01, alpha=0.8):
    ...
    if high_uncertainty_mask.any():
        # 计算对抗损失
        loss_adv = nn.CrossEntropyLoss(label_smoothing=0.1)(logits[high_uncertainty_mask],
    ...
    # 组合损失
    loss_clean = nn.CrossEntropyLoss(label_smoothing=0.1)(model(x)[0], targets)
    loss_adv = nn.CrossEntropyLoss(label_smoothing=0.1)(model(x_adv)[0], targets)
...
def train_model(config):
    ...
    change_criterion = nn.CrossEntropyLoss(label_smoothing=0.1)
    change_criterion_val = nn.CrossEntropyLoss(label_smoothing=0.0)
    class_criterion_10 = nn.CrossEntropyLoss(label_smoothing=0.1)
    class_criterion_15 = nn.CrossEntropyLoss(label_smoothing=0.1)
    ...
    for epoch in range(config['training']['epochs']):
        ...
        if val_loss < best_val_loss:
            ...
            patience_counter = 0
            # torch.save(model.state_dict(), os.path.join(config['dataset']['result_dir'], 'best_model4losses.pth'))
    ...
    model.load_state_dict(
        torch.load(os.path.join(config['dataset']['result_dir'], 'best_model4losses.pth'), weights_only=True))
    model.eval()
