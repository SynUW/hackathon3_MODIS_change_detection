...
class SparseDeformableMambaBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        print(torch.softmax(_[0]/0.001, dim=0))
...
class ChangeDetectionMamba(nn.Module):
    def __init__(self, config):
        ...
        self.channel_mamba_blocks_reverse = nn.Sequential(
            *[SparseDeformableChannelMambaBlock(
                dim=13*13,
                d_state=config['model']['d_state'],
                d_conv=config['model']['d_conv'],
        ...
        self.spectral_mamba_block_reverse = nn.Sequential(
            *[SparseDeformableChannelMambaBlock(
                dim=13*13,
                d_state=config['model']['d_state'],
                d_conv=config['model']['d_conv'],
    ...
    def forward(self, x_2010, x_2015):
        ...
        feats_2010_s_out = self.spectral_mamba_block(feats_2010_s)+self.spectral_mamba_block_reverse(feats_2010_s.flip(dims=(1,))).flip(dims=(1,))
        feats_2015_s_out = self.spectral_mamba_block(feats_2015_s)+self.spectral_mamba_block_reverse(feats_2015_s.flip(dims=(1,))).flip(dims=(1,))
