...
class SelectiveScan(torch.autograd.Function):
    ...
    def forward(ctx, u, delta, A, B, C, D=None, delta_bias=None, delta_softplus=False, nrows=1, backnrows=1, oflex=True):
        ...
        if delta_bias is not None:
            delta_bias = delta_bias.view(-1, 1).expand_as(delta)
        ...
        B_size, D_size, L_size = u.shape
        d_state = A.shape[-1]
        
        # 重塑维度以适应矩阵乘法
        x_reshaped = x.view(B_size * D_size, L_size)
        u_reshaped = u.view(B_size * D_size, L_size)
        delta_reshaped = delta.view(B_size * D_size, L_size)
        
        # 扩展 A 的维度以匹配批处理大小
        A_expanded = A.unsqueeze(0).expand(B_size * D_size, -1, -1)  # [B*D, d_state, d_state]
        
        for i in range(L_size):
            if i == 0:
                x_reshaped[:, i] = u_reshaped[:, i]
            else:
                prev_x = x_reshaped[:, i-1]  # [B*D]
                prev_x = prev_x.unsqueeze(-1).expand(-1, d_state)  # [B*D, d_state]
                A_x = torch.sum(A_expanded * prev_x.unsqueeze(1), dim=2)  # [B*D, d_state]
                x_reshaped[:, i] = u_reshaped[:, i] + delta_reshaped[:, i] * A_x.mean(dim=1)
        
        x = x_reshaped.view(B_size, D_size, L_size)
        
        # 重塑 B 和 x 的维度以匹配
        B_reshaped = B.view(B_size, -1, B.size(-1))  # [B, D, N]
        x_reshaped = x.view(B_size, D_size, L_size)  # [B, D, L]
        
        # 调整维度顺序以匹配矩阵乘法
        B_reshaped = B_reshaped.reshape(B_size, 2, 64)  # [B, 2, 64]
        x_reshaped = x_reshaped.reshape(B_size, 2, 64)  # [B, 2, 64]
        
        # 执行矩阵乘法
        y = torch.bmm(x_reshaped.transpose(1, 2), B_reshaped)  # [B, 64, 64]
        y = y.reshape(B_size, -1, L_size)  # [B, N, L]
        
        if D is not None:
            D = D.view(-1, 1).expand_as(u)
            y = y + D * u
    ...
    def backward(ctx, dout, *args):
        ...
        return du, ddelta, dA, dB, dC, dD, None, None, None, None, None
...
class SelectiveScanCore(torch.autograd.Function):
    ...
    def forward(ctx, u, delta, A, B, C, D=None, delta_bias=None, delta_softplus=False, nrows=1, backnrows=1, oflex=True):
        ...
        # 使用 matmul 替代 einsum
        y = torch.matmul(B_reshaped.transpose(1, 2), x_reshaped)  # [B, N, L]
        y = y.transpose(1, 2)  # [B, L, N]
...
def cross_selective_scan(
    ...
    x_dbl: torch.Tensor=None, 
    x_proj_weight_dbl: torch.Tensor=None,
    x_proj_bias_dbl: torch.Tensor=None,
    A: torch.Tensor=None,
    D: torch.Tensor=None,
    delta_softplus = True,
    nrows = -1,
    ...
    **kwargs,
    ...
    nrows = nrows if nrows > 0 else C
    backnrows = backnrows if backnrows > 0 else C
    
    # 处理 x_dbl 的维度
    if x_dbl is not None:
        if len(x_dbl.shape) == 4:  # 如果是 [B, C, H, W] 格式
            x_dbl = x_dbl.permute(0, 2, 3, 1).contiguous()  # [B, H, W, C]
            x_dbl = x_dbl.view(B, H*W, C)  # [B, L, C]
        else:
            x_dbl = x_dbl.permute(0, 2, 1)  # [B, L, C]
    
    # 重塑张量以进行线性变换
    x_reshaped = x.reshape(-1, x.size(-1))  # [B*L, C]
    x_dbl_reshaped = x_dbl.reshape(-1, x_dbl.size(-1))  # [B*L, C]
    
    # 处理权重矩阵的维度
    if len(x_proj_weight.shape) == 3:  # 如果是 [K, N, inner] 格式
        K, N, inner = x_proj_weight.shape
        x_proj_weight = x_proj_weight.reshape(-1, inner)  # [K*N, inner]
        x_proj_weight_dbl = x_proj_weight_dbl.reshape(-1, inner)  # [K*N, inner]
    
    # 确保输入和权重维度匹配
    if x_reshaped.size(-1) != x_proj_weight.size(-1):
        # 调整输入维度以匹配权重
        x_reshaped = x_reshaped.view(B, -1, x_reshaped.size(-1))  # [B, L, C]
        x_reshaped = x_reshaped.permute(0, 2, 1)  # [B, C, L]
        x_reshaped = x_reshaped.reshape(-1, x_reshaped.size(-1))  # [B*C, L]
        
        x_dbl_reshaped = x_dbl_reshaped.view(B, -1, x_dbl_reshaped.size(-1))  # [B, L, C]
        x_dbl_reshaped = x_dbl_reshaped.permute(0, 2, 1)  # [B, C, L]
        x_dbl_reshaped = x_dbl_reshaped.reshape(-1, x_dbl_reshaped.size(-1))  # [B*C, L]
        
        # 调整权重维度以匹配输入
        x_proj_weight = x_proj_weight.transpose(0, 1)  # [inner, K*N]
        x_proj_weight_dbl = x_proj_weight_dbl.transpose(0, 1)  # [inner, K*N]
        
        # 确保权重矩阵的维度正确
        if x_proj_weight.size(0) != x_reshaped.size(-1):
            # 调整权重矩阵的维度以匹配输入
            x_proj_weight = x_proj_weight[:x_reshaped.size(-1), :x_proj_weight.size(1)]
            x_proj_weight_dbl = x_proj_weight_dbl[:x_dbl_reshaped.size(-1), :x_proj_weight_dbl.size(1)]
            
            # 确保输出维度正确
            x_proj_weight = x_proj_weight.view(-1, 4)  # 调整为 [inner, 4]
            x_proj_weight_dbl = x_proj_weight_dbl.view(-1, 4)  # 调整为 [inner, 4]
    
    # 执行线性变换
    x_proj = F.linear(x_reshaped, x_proj_weight, x_proj_bias)  # [B*L, K*N]
    x_dbl_proj = F.linear(x_dbl_reshaped, x_proj_weight_dbl, x_proj_bias_dbl)  # [B*L, K*N]
    
    # 恢复原始维度
    x_proj = x_proj.view(B, -1, x_proj.size(-1))  # [B, L, K*N]
    x_dbl_proj = x_dbl_proj.view(B, -1, x_dbl_proj.size(-1))  # [B, L, K*N]
    
    # 调整维度顺序
    x_proj = x_proj.permute(0, 2, 1)  # [B, K*N, L]
    x_dbl_proj = x_dbl_proj.permute(0, 2, 1)  # [B, K*N, L]
    
    # 分割张量
    u, delta, A, B, C = x_proj.chunk(5, dim=1)
    u_dbl, delta_dbl, A_dbl, B_dbl, C_dbl = x_dbl_proj.chunk(5, dim=1)
    
    # 调整维度以匹配
    u = u.view(B, 2, -1)
    delta = delta.view(B, 2, -1)
    A = A.view(B, 2, -1)
    B = B.view(B, 2, -1)
    C = C.view(B, 2, -1)
    
    u_dbl = u_dbl.view(B, 2, -1)
    delta_dbl = delta_dbl.view(B, 2, -1)
    A_dbl = A_dbl.view(B, 2, -1)
    B_dbl = B_dbl.view(B, 2, -1)
    C_dbl = C_dbl.view(B, 2, -1)
    
    # 执行选择性扫描
    y = SelectiveScan.apply(
        u, delta, A, B, C, D, None, delta_softplus, nrows, backnrows, oflex
    )
    y_dbl = SelectiveScan.apply(
        u_dbl, delta_dbl, A_dbl, B_dbl, C_dbl, D, None, delta_softplus, nrows, backnrows, oflex
    )
...
class SS2D(nn.Module):
    ...
    def __initv2__(
        ...
        **kwargs,    
        ...
        # forward_type debug =======================================
        FORWARD_TYPES = dict(
            v01=partial(self.forward_corev2, force_fp32=(not self.disable_force32), SelectiveScan=SelectiveScan),
    ...
    # only used to run previous version
    def forwardv0(self, x: torch.Tensor, SelectiveScan = SelectiveScan, seq=False, force_fp32=True, **kwargs):
        x = self.in_proj(x)
