...
from mamba_spatial_sampling_channelFFN import ChangeDetectionMamba
...
config = {
    ...
    'training': {
        ...
        'to_predict_on_entire_image': False,
        'loss_weights': {
...
def adversarial_loss_2010(model, x_2010, x_2015, targets, epsilon=0.01, alpha=0.8):
    ...
    # 确保输入需要梯度
    x_adv_2010 = x_2010.clone().requires_grad_(True)  # 关键修复：显式启用梯度
    x_adv_2015 = x_2015.clone().requires_grad_(True)  # 关键修复：显式启用梯度

    # 原始预测
    logits = model(x_adv_2010, x_adv_2015)
    probs = torch.softmax(logits['class_2010'], dim=1)
    aleatoric = -torch.sum(probs * torch.log(probs + 1e-10), dim=1)

    # 仅对高不确定性样本生成对抗样本
    # high_uncertainty_mask = aleatoric > aleatoric.median()
    # 仅选择不确定性最高的top_k个样本
    _, top_indices = torch.topk(aleatoric, k=14, largest=True)  # largest=True选择最大值
    ...
    if high_uncertainty_mask.any():
        # 计算对抗损失
        loss_adv = nn.CrossEntropyLoss()(logits['class_2010'][high_uncertainty_mask],
                                                            targets[high_uncertainty_mask])
        grad = torch.autograd.grad(loss_adv, x_adv_2010, retain_graph=True)[0]  # 计算梯度

        # 生成对抗样本（仅修改高不确定性样本）
        with torch.no_grad():
            x_adv_2010[high_uncertainty_mask] = x_adv_2010[high_uncertainty_mask] + epsilon * grad[high_uncertainty_mask].sign()

    # 组合损失
    loss_clean = nn.CrossEntropyLoss()(model(x_2010, x_2015)['class_2010'], targets)
    loss_adv = nn.CrossEntropyLoss()(model(x_2010, x_2015)['class_2010'], targets)
    return alpha * loss_clean + (1 - alpha) * loss_adv

def adversarial_loss_2015(model, x_2010, x_2015, targets, epsilon=0.01, alpha=0.8):
    # baseline 是96.15，使用alpha=0.3得到的 test acc是96.29; 0.8-> 96.25
    # 0.01, 0.8并且使用20%的不确定性样本对抗，得到的acc是96.35%
    #  0.01, 0.8并且使用10%的不确定性样本对抗，validation acc更高，但是test低了
    # 确保输入需要梯度
    ...
    # 原始预测
    logits = model(x_adv_2010, x_adv_2015)
    probs = torch.softmax(logits['class_2015'], dim=1)
    aleatoric = -torch.sum(probs * torch.log(probs + 1e-10), dim=1)
    ...
    if high_uncertainty_mask.any():
        # 计算对抗损失
        loss_adv = nn.CrossEntropyLoss()(logits['class_2015'][high_uncertainty_mask],
                                                            targets[high_uncertainty_mask])
        grad = torch.autograd.grad(loss_adv, x_adv_2015, retain_graph=True)[0]  # 计算梯度

        # 生成对抗样本（仅修改高不确定性样本）
        with torch.no_grad():
            x_adv_2015[high_uncertainty_mask] = x_adv_2015[high_uncertainty_mask] + epsilon * grad[high_uncertainty_mask].sign()

    # 组合损失
    loss_clean = nn.CrossEntropyLoss()(model(x_2010, x_2015)['class_2015'], targets)
    loss_adv = nn.CrossEntropyLoss()(model(x_2010, x_2015)['class_2015'], targets)
    return alpha * loss_clean + (1 - alpha) * loss_adv
...
def train_model(config):
    ...
    for epoch in range(config['training']['epochs']):
        ...
        for batch in train_loader:
            ...
            class_loss_2010 = adversarial_loss_2010(model, x_2010, x_2015, class_labels_2010)
            class_loss_2015 = adversarial_loss_2015(model, x_2015, x_2015, class_labels_2015)
        ...
        if val_loss < best_val_loss:
            ...
            patience_counter = 0
            torch.save(model.state_dict(), os.path.join(config['dataset']['result_dir'], 'best_model_2.pth'))
    ...
    model.load_state_dict(
        torch.load(os.path.join(config['dataset']['result_dir'], 'best_model_2.pth'), weights_only=True))
    model.eval()
