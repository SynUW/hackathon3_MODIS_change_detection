...
def compute_weights(feat, dim):
    # feat: [B, C, H*W]
    mean_pool = feat.mean(dim=dim)  # [B, H*W]
    max_pool = feat.amax(dim=dim)  # [B, H*W]
    weights = torch.sigmoid(mean_pool + max_pool)  # [B, H*W]
    return weights
...
class ChangeDetectionMamba(nn.Module):
    ...
    def forward(self, x_2010, x_2015):
        ...
        feats_2010_t_out = self.temporal_mamba(feats_2010_t)
        feats_2015_t_out = self.temporal_mamba(feats_2015_t)

        # t1, t1, t1, ..., t2, t2, t2, ...
        t_2010_score = compute_weights(feats_2010_t_out, -1).unsqueeze(-1).repeat(1, 1, C//23).transpose(1, 2).reshape(B, -1).unsqueeze(-1)  # B, C, 1
        t_2015_score = compute_weights(feats_2015_t_out, -1).unsqueeze(-1).repeat(1, 1, C//23).transpose(1, 2).reshape(B, -1).unsqueeze(-1)

        feats_2010_t_out = feats_2010_t_out.reshape(B, 23, C//23, H*W).reshape(B, C, H*W)
        feats_2015_t_out = feats_2015_t_out.reshape(B, 23, C//23, H*W).reshape(B, C, H*W)
        ...
        feats_2010_s_out = self.spectral_mamba(feats_2010_s)
        feats_2015_s_out = self.spectral_mamba(feats_2015_s)

        # c1, c2, ...c6, c1, c2, ... c6
        s_2010_score = compute_weights(feats_2010_s_out, -1).repeat_interleave(23, dim=-1).unsqueeze(-1)  # B, C, 1
        s_2015_score = compute_weights(feats_2015_s_out, -1).repeat_interleave(23, dim=-1).unsqueeze(-1)

        feats_2010_s_out = feats_2010_s_out.reshape(B, C//23, 23, H*W).permute(0, 2, 1, 3).reshape(B, C, H*W)
        feats_2015_s_out = feats_2015_s_out.reshape(B, C//23, 23, H*W).permute(0, 2, 1, 3).reshape(B, C, H*W)

        feats_2010_spa_out = self.mamba_blocks(feats_2010.reshape(B, C, H*W).permute(0, 2, 1))
        feats_2015_spa_out = self.mamba_blocks(feats_2015.reshape(B, C, H*W).permute(0, 2, 1))

        spatial_2010_score = compute_weights(feats_2010_spa_out, -1).unsqueeze(1)  # B, 1, H*W
        spatial_2015_score = compute_weights(feats_2015_spa_out, -1).unsqueeze(1)

        feats_2010_spa_out = feats_2010_spa_out.permute(0, 2, 1).reshape(B, C, H*W)
        feats_2015_spa_out = feats_2015_spa_out.permute(0, 2, 1).reshape(B, C, H*W)

        # feature fusion
        feats_2010_out = (feats_2010_t_out*spatial_2010_score*s_2010_score +
                          feats_2010_s_out*spatial_2010_score*t_2010_score +
                          feats_2010_spa_out*t_2010_score*s_2010_score)
        feats_2015_out = (feats_2015_t_out*spatial_2015_score*s_2015_score +
                          feats_2015_s_out * spatial_2015_score * t_2015_score +
                          feats_2015_spa_out * t_2015_score * s_2015_score)
