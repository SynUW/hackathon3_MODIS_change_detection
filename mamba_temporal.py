...
class ChangeDetectionMamba(nn.Module):
    def __init__(self, config):
        ...
        self.mamba_blocks_2 = nn.Sequential(
            *[SparseDeformableChannelMambaBlock(
                dim=13*13*24,
                d_state=config['model']['d_state'],
                d_conv=config['model']['d_conv'],
        ...
        self.change_head = nn.Sequential(
            nn.Linear(2 * 23, config['model']['hidden_dim']),
            nn.GELU(),
            nn.Linear(config['model']['hidden_dim'], 2)
        )

        self.class_head_2010 = nn.Sequential(
            nn.Linear(23, config['model']['hidden_dim']),
            nn.GELU(),
        ...
        self.input_proj = nn.Linear(13*13, 13*13*3)  # 升维 x
        self.ffn = nn.Sequential(
            nn.Linear(13*13, 13*13*3),
            nn.GELU()
        )
    ...
    def forward(self, x_2010, x_2015):
        ...
        feats_2010 = feats_2010.reshape(B, 23, C//23, H*W).reshape(B, 23, C//23*H*W)
        feats_2015 = feats_2015.reshape(B, 23, C//23, H*W).reshape(B, 23, C//23*H*W)

        feats_2010 = self.mamba_blocks(feats_2010).permute(0, 2, 1)
        feats_2015 = self.mamba_blocks_2(feats_2015).permute(0, 2, 1)
