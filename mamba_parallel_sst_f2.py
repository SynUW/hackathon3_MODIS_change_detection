...
class ChangeDetectionMamba(nn.Module):
    ...
    def forward(self, x_2010, x_2015):
        ...
        # 新增对比学习输出
        contrast_feat_2010 = self.contrast_head(feats_2010_out.mean(dim=[1]))  # [B, proj_dim]
        contrast_feat_2015 = self.contrast_head(feats_2015_out.mean(dim=[1]))

        return {
            'change': change_logits,
            'class_2010': class_logits_2010,
            'class_2015': class_logits_2015,
            'contrast_2010': contrast_feat_2010,  # 新增
            'contrast_2015': contrast_feat_2015  # 新增
        }
