...
from comparison_models.ChangeSparse import ChangeSparseM2M
...
def predict_full_image(model, dataset, config, device):
    ...
    with torch.no_grad():
        for batch in tqdm(loader, desc="Predicting full image"):
            ...
            outputs = model(torch.cat([x_2010, x_2015], dim=1))
...
def train_model(config):
    ...
    model = ChangeSparseM2M(config_changesparse).to(device)
    ...
    for epoch in range(config['training']['epochs']):
        ...
        for batch in train_loader:
            ...
            # 将两个时间点的输入拼接在一起
            x = torch.cat([x_2010, x_2015], dim=1)  # [B, 276, 13, 13]
            # 将拼接后的输入拆分为两个时间点的输入
            x1 = x[:, :138, :, :]  # 2010年的输入
            x2 = x[:, 138:, :, :]  # 2015年的输入
            outputs = model(torch.cat([x1, x2], dim=1))
