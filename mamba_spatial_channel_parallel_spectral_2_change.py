...
class ChangeDetectionMamba(nn.Module):
    def __init__(self, config):
        ...
        self.feature_extractor_spectral = nn.Sequential(
            nn.Conv2d(config['model']['num_bands'], config['model']['hidden_dim'],
                      kernel_size=3, padding=1, groups=6),
            nn.GroupNorm(num_channels=config['model']['hidden_dim'], num_groups=12),
            nn.GELU(),
            nn.Conv2d(config['model']['hidden_dim'], config['model']['hidden_dim'],
                      kernel_size=3, padding=1, groups=12),
            nn.GroupNorm(num_channels=config['model']['hidden_dim'], num_groups=12),
            nn.GELU()
        )
        ...
        self.mamba_blocks_change = nn.Sequential(
            *[SparseDeformableMambaBlock(
                dim=config['model']['hidden_dim'],
                d_state=config['model']['d_state'],
                d_conv=config['model']['d_conv'],
        ...
        self.channel_mamba_blocks_change = nn.Sequential(
            *[SparseDeformableChannelMambaBlock(
                dim=13*13,
                d_state=config['model']['d_state'],
                d_conv=config['model']['d_conv'],
        ...
        self.spectral_mamba_block_chnage = nn.Sequential(
            *[SparseDeformableChannelMambaBlock(
                dim=13*13,
                d_state=config['model']['d_state'],
                d_conv=config['model']['d_conv'],
    ...
    def forward(self, x_2010, x_2015):
        ...
        C_org = x_2010.shape[1]
        feats_2010_s = x_2010.reshape(B, 23, C_org // 23, H, W).permute(0, 2, 1, 3, 4).reshape(B, C_org, H, W)
        feats_2015_s = x_2015.reshape(B, 23, C_org // 23, H, W).permute(0, 2, 1, 3, 4).reshape(B, C_org, H, W)
        ...
        feats_change_c_out = self.channel_mamba_blocks_change(torch.abs(feats_2010_c_out-feats_2015_c_out))

        feats_2010_s_out = self.spectral_mamba_block(feats_2010_s)
        feats_2015_s_out = self.spectral_mamba_block(feats_2015_s)
        feats_change_s_out = self.spectral_mamba_block_chnage(torch.abs(feats_2010_s_out-feats_2015_s_out))
        ...
        feats_change_out = self.mamba_blocks_change(torch.abs(feats_2010-feats_2015))
        ...
        change_feats = (feats_change_out+(feats_change_c_out+feats_change_s_out).permute(0, 2, 1)).mean(dim=1)
