...
class ChangeDetectionMamba(nn.Module):
    def __init__(self, config):
        ...
        self.channel_shuffle = nn.Sequential(
            nn.Conv2d(552, 552, kernel_size=1),
            nn.GELU()
        )
    ...
    def forward(self, x_2010, x_2015):
        ...
        feats_2010 = self.channel_shuffle(feats_2010.reshape(B, C, H, W)).reshape(B, C, H*W)
        feats_2015 = self.channel_shuffle(feats_2015.reshape(B, C, H, W)).reshape(B, C, H*W)
...
if __name__ == "__main__":
    ...
    dummy_input_2010 = torch.randn(512, 138, 13, 13).to(device)
    dummy_input_2015 = torch.randn(512, 138, 13, 13).to(device)
