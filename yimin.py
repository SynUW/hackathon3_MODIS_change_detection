...
class SparseDeformableMambaBlock(nn.Module):
    def __init__(self, dim, d_state=16, d_conv=4, expand=2, sparsity_ratio=0.3, drop_rate=0.3, spa=True, spe=True, temp=False):
        super().__init__()
        self.dim = dim
        self.d_state = d_state
        self.expand = expand
        ...
        # self.norm = nn.Sequential(nn.Conv1d(dim, dim, kernel_size=1, stride=1, padding=0, bias=False),
        #                        DyT(dim),
        #                        nn.GELU())
    ...
    def forward(self, x, spa=True, spe=True, temp=False):
        ...
        if spa is False and spe is True and temp is False:
            ...
            #loc1 = loc * 6
            # if block_idx == 1:
            #     xx = self.season(x_proj)
            #     x_proj = xx.permute(0, 2, 1)
            #     x_proj = self.down(x_proj)
            #     x_sparse, loc = self.attn(x_proj)
            #     loc1 = loc * 4
            #x_proj_reshaped = x_proj.view(B, 23, 6, -1)
            #x_proj = x_proj.permute(0, 2, 1)  # [1024, 169, 138]
            #x_proj = self.down(x_proj)  # [1024, 23, 1, 338]
            #x_proj = x_proj_reshaped.squeeze(dim=2)  # [1024, 23, 338]
            #x_proj = self.down(x_proj)
            ...
            # channel_var = x_proj.var(dim=(0, 2))  # 输出 shape: (batchsize  138)
            # most_informative_channel = torch.argmax(channel_var).item()
            #
            # center_idx = most_informative_channel
            # center = x_proj[:, center_idx:center_idx + 1, :]
            # #
            # x_proj_norm = F.normalize(x_proj, p=2, dim=-1)  # [B, L, D]
            # center_norm = F.normalize(center, p=2, dim=-1)  # [B, 1, D]
            ...
            #x_processed = self.attn(x_processed)
            #]x_processed = x_processed.permute(0, 2, 1)  # [1024, 23, 169]

            #output = torch.repeat_interleave(x_processed, repeats=6, dim=1)
            # Combine with residual
            # x_processed = x_processed + batched_index_select(residual.reshape(B, L, C), 1, topk_idx)
...
class SpatialSpectralMambaBlock(nn.Module):
    def __init__(self, dim: int, patch_size: int, num_heads = 8, block_idx = 0):
        ...
        self.meanpool = nn.MaxPool2d(kernel_size=patch_size, return_indices=True)
    ...
    def forward(self, x):
        ...
        #x_spectral = x_spatial1.reshape(B, -1, T, H, W)
        x_spectral, idx = self.meanpool(x_spatial1) # [B, C, 1, 1]
        x_spectral = x_spectral.reshape(B, -1, T, 1, 1).reshape(B, -1, T)
        #x_spectral = x_spectral.reshape(B, C, H * W)
        ...
        #x_spectral = x_temporal.reshape(B, num_spe, -1)
        #x_spectral = spectral_in.reshape(B, C, H * W)
        #x_spectral, _ = self.spectral_mamba(x_spectral, spa=False, spe=True)
        #x_spectral = x_spectral.reshape(B, num_spe, T, H, W).reshape(B, -1, H, W)
        #fusion = self.attn2(x_spatial.reshape(B, C, -1).permute(0,2,1), x_spectral.reshape(B, C, -1).permute(0,2,1))
        #fusion = fusion.reshape(B, C, H, W)
        #x = self.stem(x_temporal)
...
class ChangeDetectionMamba(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        #self.T = nn.Parameter(torch.from_numpy(TRANSITION_MATRIX))
        ...
        # self.log_sigma4 = nn.Parameter(torch.zeros(1))
        # self.log_sigma5 = nn.Parameter(torch.zeros(1))
        # self.log_sigma6 = nn.Parameter(torch.zeros(1))
        ...
        self.spectral_blocks = nn.Sequential(
            *[SpatialSpectralMambaBlock(dim=config['model']['hidden_dim'], patch_size=13, block_idx = i) for i in range(1)]
        )
        ...
        #     nn.Linear(config['model']['hidden_dim'], config['model']['num_classes'])
        # )
        #self.cd_head = SimpleChangeDetectionHead(in_channels=config['model']['hidden_dim'], hidden_channels=config['model']['hidden_dim'],)
    ...
    def forward(self, x_2010, x_2015):
        ...
        #feats_2010 = feats_2010.permute(0, 2, 3, 1).reshape(B, H*W, C)
        ...
        # x_spa1 = x_spa1.reshape(B, C, H * W).permute(0, 2, 1).mean(dim=1)
        # x_spa2 = x_spa2.reshape(B, C, H * W).permute(0, 2, 1).mean(dim=1)
        # x_spe1 = x_spe1.reshape(B, C, H * W).permute(0, 2, 1).mean(dim=1)
        # x_spe2 = x_spe2.reshape(B, C, H * W).permute(0, 2, 1).mean(dim=1)
        # pdb.set_trace()
        # feats_2010 = torch.exp(sim2010).unsqueeze(dim=2) * feats_2010
        # feats_2015 = torch.exp(sim2015).unsqueeze(dim=2) * feats_2015
        ...
        #change_logits = self.cd_head(x_spa1, x_spa2, x_spe1, x_spe2)
        ...
        #proj_2010 = self.proj_head2010(pooled_2010)
        #proj_2015 = self.proj_head2015(pooled_2015)
        return {
...
def train_model(config):
    ...
    for epoch in range(config['training']['epochs']):
        ...
        for batch in train_loader:
            ...
            outputs = model(x_2010/255, x_2015/255)
            ...
            loss = partial_alignment_loss(outputs['class_2010'], outputs['class_2015'], class_labels_2010, class_labels_2015, temperature=0.3)
            total_loss = (
            ...
            if epoch % 5 == 0 and batch_idx == 1:
                ...
                print(f"LC2010: {class_loss_2010.item():.4f} ({landcover_w*class_loss_2010.item()/total_loss.item():.1%})")
                print(f"LC2015: {class_loss_2015.item():.4f} ({landcover_w*class_loss_2015.item()/total_loss.item():.1%})")
    ...
    model.load_state_dict(torch.load(os.path.join(config['dataset']['result_dir'], 'best_model.pth'), weights_only=True))
    model.eval()
