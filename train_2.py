...
# from MambaCD.changedetection.models.STMambaSCD_13by13 import STMambaSCD_NoDownsample
...
def get_model_name_from_import():
    ...
    with open(__file__, 'r') as f:
        for line in f:
            if 'from MambaCD.changedetection.models.' in line and 'import' in line:
                import_line = line.strip()
                break
    
    if import_line:
        # 从 "from comparison_models.XXX import YYY" 中提取 XXX
        model_name = import_line.split('from MambaCD.changedetection.models.')[1].split(' import')[0]
        return model_name
...
def train_model(config):
    ...
    model = STMambaSCD_NoDownsample(**model_kwargs).to(device)
