...
class ChannelWiseAttention(nn.Module):
    ...
    def forward(self, x):
        B, C, X = x.shape  # 输入形状: [batch, channels, features]
        h = self.num_heads
        k1 = max(1, int(X * self.sparsity_ratio))  # 基于特征维度计算稀疏数

        # 修正reshape操作
        # 首先确保d_model维度匹配
        assert x.size(2) == self.d_model, f"Expected feature dimension {self.d_model}, got {x.size(2)}"

        # 转换为 [B*C, X, d_model] 形状
        x_reshaped = x.permute(0, 2, 1).reshape(-1, X, self.d_model)

        # 投影QKV
        q = self.to_q(x_reshaped)
        k = self.to_k(x_reshaped)
        v = self.to_v(x_reshaped)
        ...
        mask1 = torch.zeros_like(attn).scatter_(-1, idx2.expand(-1, attn.shape[1], -1), 1.0)

        col_variance = attn.mean(dim=-1, keepdim=True)
        _, idx = torch.topk(col_variance, k=k1, dim=1)
        mask2 = torch.zeros_like(attn).scatter_(1, idx.expand(-1, -1, attn.shape[2]), 1.0)

        M = (mask1 + mask2).clamp(0, 1)
        sparse_attn = attn * M

        # 应用注意力
        out = einsum('b i j, b j d -> b i d', sparse_attn, v)
        out = rearrange(out, '(b h) n d -> b n (h d)', h=h)  # [B*C, X, d_model]

        # 恢复原始形状 [B, C, X, d_model]
        out = out.reshape(B, X, C, self.d_model).permute(0, 2, 1, 3)

        # 计算特征重要性 (沿X维度)
        feature_importance = out.mean(dim=[1, 3])  # [B, X]

        # 选择top-k特征
        _, loc = torch.topk(feature_importance, k=k1, dim=-1)

        # 收集选中的特征 [B, C, k1, d_model]
        selected_features = torch.stack([
            out[i, :, loc[i]] for i in range(B)
        ], dim=0)

        # 将选中的特征展平为 [B, C, k1 * d_model]
        sparse_feat = selected_features.reshape(B, C, -1)

        return sparse_feat, loc, k1
...
class ChangeDetectionMamba(nn.Module):
    ...
    def forward(self, x_2010, x_2015):
        ...
        pooled_2010 = (feats_2010+feats_2010_c_out.permute(0, 2, 1)).mean(dim=1)
        pooled_2015 = (feats_2015+feats_2015_c_out.permute(0, 2, 1)).mean(dim=1)
...
if __name__ == "__main__":
    ...
    dummy_input_2010 = torch.randn(512, 138, 13, 13).to(config['training']['device'])
    dummy_input_2015 = torch.randn(512, 138, 13, 13).to(config['training']['device'])
    model = ChangeDetectionMamba(config).to(config['training']['device'])
