...
class SparseDeformableMambaBlock(nn.Module):
    def __init__(self, dim, d_state=16, d_conv=4, expand=2, sparsity_ratio=0.3, drop_rate=0.3, topk=False):
        super().__init__()
        self.dim = dim
        self.d_state = d_state
        self.expand = expand
...
class ChangeDetectionMamba(nn.Module):
    ...
    def forward(self, x_2010, x_2015):
        ...
        feats_2010 = self.channel_mamba_blocks(feats_2010.flip(dims=[1]))
        feats_2015 = self.channel_mamba_blocks(feats_2015.flip(dims=[1]))
